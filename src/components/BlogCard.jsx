...
const BlogCard = ({ d }) => {
  ...
  return (
    ...
    <div  className={` ml-8 ${darkMode ? 'dark-mode' : 'light-mode'} dark:hover:bg-red-600 ${darkMode ? ' hover:bg-black' : 'hover:bg-gray-100'}   transition-all duration-100 dark:bg-gray-800 border-gray-200 dark:border-gray-800 dark:text-gray-100 p-4 rounded-xl border min-w-80  max-w-80 sm:max-w-xl`}>
      ...
      <p className={`text-black dark:text-gray-300 block text-sm max-w-70 min-w-70  sm:min-w-[505px] ${darkMode ? 'dark-mode' : 'light-mode'} leading-snug mt-3`}>{cleanString}</p>
     {imageUrl &&  <img className="mt-4 mb-2 w-full h-64 object-contain aspect-square rounded-2xl border border-gray-100 dark:border-gray-700" src={imageUrl } alt="Tweet"/>}
